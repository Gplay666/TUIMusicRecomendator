...
# data = measure_performance(AVLTree, sizes)
#
# # Вывод таблицы
# print("N\tHeight\tInsert(μs)\tSearch(μs)")
# for size in sizes:
#     print(f"{size}\t{data[size]['height']:.1f}\t{data[size]['insert_time']:.2f}\t\t{data[size]['search_time']:.2f}")
#
# # Построение графиков
...
# # Высота
# x = sizes
# y_height = [data[s]['height'] for s in sizes]
# y_log2 = [np.log2(s) for s in sizes]
# y_144log2 = [1.44 * np.log2(s) for s in sizes]
# ax1.plot(x, y_height, 'o-', label='эксперимент')
# ax1.plot(x, y_log2, '--', label='log2(N)')
# ax1.plot(x, y_144log2, '--', label='1.44*log2(N)')
# ax1.set_xlabel('N')
# ax1.set_ylabel('Высота')
# ax1.set_title('Высота AVL-дерева')
# ax1.legend()
# ax1.grid(True)
#
# # Время
# y_insert = [data[s]['insert_time'] for s in sizes]
# y_search = [data[s]['search_time'] for s in sizes]
# ax2.plot(x, y_insert, 'o-', label='вставка')
# ax2.plot(x, y_search, 's-', label='поиск')
# ax2.set_xlabel('N')
# ax2.set_ylabel('Время (мкс)')
# ax2.set_title('Среднее время операций')
# ax2.legend()
# ax2.grid(True)
#
# plt.tight_layout()
